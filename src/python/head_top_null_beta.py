...
def make_df_beta(question_15_or_more_path, calculate_beta_path, head_top_null_path):
    ...
    beta = pd.read_csv(f'{calculate_beta_path}QOL_covariance_correlation_beta.tsv.gz', sep='\t',
                     encoding='utf-8', compression='gzip')
    # Merge files
    QOL_beta = pd.merge(QOL[['project_pseudo_id', 'responsedate', 'qualityoflife']], beta[['project_pseudo_id', 'beta']], how='outer', on=['project_pseudo_id'])
...
def read_df_beta(head_top_null_path, df):
    ...
    head_tail = pd.merge(head_nn[['project_pseudo_id', 'responsedate', 'qualityoflife', 'beta', 'beta_type']], tail_nn[['project_pseudo_id', 'responsedate', 'qualityoflife', 'beta', 'beta_type']], how='outer', on=['project_pseudo_id', 'responsedate', 'qualityoflife', 'beta', 'beta_type'])
    ...
    # Merge
    head_tail_null_df = pd.merge(head_tail[['project_pseudo_id', 'responsedate', 'qualityoflife', 'beta', 'beta_type']], head_df_abs[['project_pseudo_id', 'responsedate', 'qualityoflife', 'beta', 'beta_type']], how='outer', on=['project_pseudo_id', 'responsedate', 'qualityoflife', 'beta', 'beta_type'])
    # Write files
