...
def select_label_depressive(before_mini_df):
    ...
    # Merge
    before_mini_df = pd.merge(before_mini_df,
                              major_depressive_episode[['project_pseudo_id', 'major_depressive_episode']],
...
def select_label_anxiety(before_mini_df):
    ...
    # Merge
    before_mini_df = pd.merge(before_mini_df,
                              generalized_anxiety_disorder[['project_pseudo_id', 'generalized_anxiety_disorder']],
                              on=['project_pseudo_id'], how='outer')
...
def main():
    ...
    merge_before_between['generalized_anxiety_disorder'] = merge_before_between['generalized_anxiety_disorder'].fillna(
        0)
    merge_before_between.to_csv(f"{mini_path}between_before_mini.tsv.gz", sep='\t',
                                encoding='utf-8', compression='gzip', index=False)
    print('DONE: mini_data.py')
