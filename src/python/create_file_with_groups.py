...
def merge_questdata_with_other(data_QOL, create_file_with_groups_path, question_15_or_more_path):
    ...
    df_well.to_csv(f'{create_file_with_groups_path}num_quest_1_filter_plus_wellbeing.tsv.gz', sep='\t', encoding='utf-8',
...
def add_general_health(create_file_with_groups_path, path_directory, config):
    ...
    """
    # Read file
    df_QOL = pd.read_csv(f'{create_file_with_groups_path}merge_15ormore_household_media.tsv.gz', sep='\t', encoding='utf-8',
                            compression='gzip')
    ...
    for i in range(1,10):
        ...
        for col in df.columns:  
            if interesting_column in col:
                ...
                variables_df = pd.read_csv(f"{config['path_questionnaire_enumerations']}covq_q_t0{i}_enumerations.csv", sep=',', encoding='utf-8')
...
def add_income(create_file_with_groups_path, path_directory, config):
    ...
    """
    # Read file
    df_QOL = pd.read_csv(f'{create_file_with_groups_path}merge_15ormore_household_media_generalhealth.tsv.gz', sep='\t', encoding='utf-8',
                            compression='gzip')
    inter_column = list()
...
def add_educational_level(create_file_with_groups_path, path_directory, config):
    ...
    """
    # Read file
    df_QOL = pd.read_csv(f'{create_file_with_groups_path}merge_15ormore_household_media_generalhealth_income.tsv.gz', sep='\t', encoding='utf-8',
                            compression='gzip')
    list_columns = list()
    ...
    df_QOL.to_csv(f'{create_file_with_groups_path}merge_15ormore_household_media_generalhealth_income_education.tsv.gz', sep='\t',
                         encoding='utf-8', compression='gzip', index=False)
    # Call sep_data: also save the different categories in a separate file
