...
def get_data_ready(tests_over_groups_and_beta_path, calculate_beta_path, create_file_with_groups_path,
                   question_15_or_more_path, BFI_path):
    """

    """
    # Read files
    df_QOL = pd.read_csv(
        f'{create_file_with_groups_path}merge_15ormore_household_media_generalhealth_income_education.tsv.gz',
    ...
    # Read file
    covariance_df = pd.read_csv(f'{calculate_beta_path}QOL_covariance_correlation_beta.tsv.gz', sep='\t',
                                encoding='utf-8',
                                compression='gzip')
    # Remove 'Unnamed: 0'
...
def merge_other_data(df_QOL, tests_over_groups_and_beta_path, resilience_path, mini_path, head_top_null_path):
    # Read files
    if df_QOL.empty:
        df_QOL = pd.read_csv(f'{tests_over_groups_and_beta_path}merge_no_mini_last.tsv.gz', sep='\t', encoding='utf-8',
                             compression='gzip')
    resilience = pd.read_csv(f"{resilience_path}resilience.tsv.gz", sep='\t', encoding='utf-8', compression='gzip')
    mini = pd.read_csv(f"{mini_path}between_before_mini.tsv.gz", sep='\t', encoding='utf-8', compression='gzip')
    # Merge files
    df_QOL = pd.merge(df_QOL, resilience, on=['project_pseudo_id'], how='outer')
    ...
    df_beta_type = pd.read_csv(f'{head_top_null_path}head_tail_null_10_beta_abs.tsv.gz', sep='\t', encoding='utf-8',
                               compression='gzip')
    ...
    df_QOL_select = df_QOL[['project_pseudo_id', 'responsedate', 'qualityoflife',
                            ...
                            'beta_type']]
    # Save files
    df_QOL_select.to_csv(f'{tests_over_groups_and_beta_path}QOL_selected_columns_withbetatypes.tsv.gz', sep='\t',
                         encoding='utf-8',
                         compression='gzip', index=False)
    # Filter file
    ...
    df_QOL_select_filter.to_csv(
        f'{tests_over_groups_and_beta_path}QOL_selected_columns_withbetatypes_noageresponsequalnumquest.tsv.gz',
        sep='\t', encoding='utf-8',
        compression='gzip', index=False)
