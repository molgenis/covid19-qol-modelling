...
def linear_regression(X_train, X_test, y_train, y_test, myfile, values):
    ...
    myfile.writelines(f'linear regression\t{values}\t{line_reg.intercept_}\t{line_reg.coef_}\t{lin_reg_rmse}\t{lin_reg_acc}\n')
...
def polynomial_linear_regression(X_train, X_test, y_train, y_test, myfile, values):
    ...
    myfile.writelines(f'Plynomial Regression\t{values}\t{poly_reg_model.intercept_}\t{poly_reg_model.coef_}\t{poly_reg_rmse}\t{poly_reg_acc}\n')
...
def run_models(qol_df, total_df, rolling_avg_temp_col, daylight_hours_col, myfile, create_model):
    ...
    for values in value_list:
        ...
        poly_reg, poly_reg_model, myfile = polynomial_linear_regression(X_train, X_test, y_train, y_test, myfile, values)
