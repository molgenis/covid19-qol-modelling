...
def get_data(path_directory, information_num_quest_path):
    ...
    """
    # Get data out of lifelines questionnaires
    ...
    for files in os.listdir(path_directory):
        if files.startswith('cov'):
            ...
            df_quest = pd.merge(df_quest, df, how='outer',
                                on=['project_pseudo_id', 'responsedate', 'gender', 'age', 'num_quest'])
    # Save file
...
def filter_data(df_quest, information_num_quest_path):
    ...
    # Filter data on num_entered more or equal to 15
    ...
    df_quest_only_gender_filter.rename(columns={df_quest_only_gender_filter.columns[2]: "gender_count_filter"},
                                       inplace=True)
    ...
    df_quest_only = pd.merge(df_quest_only, df_age[['num_quest', 'gender', 'avg_age']], how='outer',
                             on=['num_quest', 'gender'])
