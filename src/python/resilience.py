...
def results_data(path_questionnaire_results, resilience_id, resilience_path):
    ...
    resilience_df: Dataframe with the resilience answers
    """
    ...
    for files in os.listdir(path_questionnaire_results):
        if files.startswith('cov'):
            ...
            if len(resilience_col) > 0:
                ...
                resilience_df = pd.merge(resilience_df, df[['project_pseudo_id', filenum]], on=['project_pseudo_id'],
                                         how='outer')
    # Save file
...
def calculate_resilience(resilience_path, resilience_df):
    ...
    if resilience_df.empty:
        resilience_df = pd.read_csv(f"{resilience_path}resilience_without_mean.tsv.gz", sep='\t', encoding='utf-8',
                                    compression='gzip')
    ...
    # Make integers of values in column
    ...
    # Calculate mean and median of the resilience
